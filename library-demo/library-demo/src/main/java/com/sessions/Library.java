...
public class Library {
    ...
    public Receipt returnBook(RentedBook rentedBook, Double amountGiven) {

        Double actualAmount=calculateBookRent(rentedBook);

        if (actualAmount>amountGiven)
            throw new LowerAmountException("Amount Given Should Atleast be"+actualAmount);

        Receipt receipt=new Receipt(rentedBook.getBook().getName(),amountGiven,actualAmount);
